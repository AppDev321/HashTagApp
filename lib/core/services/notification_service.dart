...
class NotificationService {
  ...
  NotificationService._internal() {
    ...
    plugin.initialize(initializationSettings, onDidReceiveNotificationResponse: onDidReceiveNotificationResponse);
  }
  ...
  Future<void> newNotification(String title, String msg, String? payload, bool vibration, {bool sound = true}) async {
    ...
    var iosNotificationDetails =
        DarwinNotificationDetails(presentAlert: true, presentSound: sound, presentBadge: true, categoryIdentifier: channelName);
    ...
    try {
      ...
      debugPrint("notificaiton exception ==>$ex");
    }
  }
  ...
  Future<void> scheduledNotification(String title, String msg, bool vibration, {bool sound = true}) async {
    ...
    try {
      await plugin.zonedSchedule(notificationID++, title, msg, tz.TZDateTime.now(tz.local).add(const Duration(seconds: 30)), notificationDetails,
  ...
  Future<void> scheduleDailyNotification(bool vibration,
      {bool sound = true, int id = 0, String? hourAndMinute /*Like 10:30*/, String? title, String? body, String? payload}) async {
    // Define vibration pattern
    var vibrationPattern = Int64List(4);
    vibrationPattern[0] = 0;
    vibrationPattern[1] = 1000;
    ...
    await plugin.zonedSchedule(minute * 2, title, body, nextTimeToRun, notificationDetails,
  ...
  Future<void> scheduleDailyNotifications({List<String>? hourAndMinuteList /*Like 10:30*/, String? title, String? body, String? payload}) async {
    ...
    hourAndMinuteList?.forEach((hourAndMinute) {
      scheduleDailyNotification(false, hourAndMinute: hourAndMinute, title: "$title -- $hourAndMinute", body: body, payload: payload);
    });
  }
